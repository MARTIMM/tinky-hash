...
subtest 'run errors', {
  ...
  is-deeply $th1.next-states>>.name.sort, (<a>,), "next: {$th1.next-states>>.name}";

  try-run {$th1.go-state('c');}, /:s No Transition for \'c\' to \'c\'/;
  try-run {$th1.workflow('wf2');}, /:s Workflow name \'wf2\' not defined/;
  ...
  /:s Workflow \'wf1\' defined before/;

  /:s State enter method \'m4\' not found in Tinky\:\:Hash/;
  class C2 is Tinky::Hash {
    submethod BUILD ( ) {
      self.from-hash(
        :config( {
            :states([< a b p>]),
